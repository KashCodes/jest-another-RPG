...
function Game() {
  ...
  // checks to see if it is the end of the battle 
  Game.prototype.checkEndOfBattle = function() {

    // first checks to see if both players are alive. 
    if (this.player.isAlive() && this.currentEnemy.isAlive()) {
      this.isPlayerTurn = !this.isPlayerTurn;
      this.battle();
    }
    // if player is alive and enemy is dead, log it first, then... 
    else if (this.player.isAlive() && !this.currentEnemy.isAlive()) {
      console.log(`You've defeated the ${this.currentEnemy.name}`);
      
      // reward player a potion for defeating enemy
      this.player.addPotion(this.currentEnemy.potion);
      console.log(`${this.player.name} found a ${this.currentEnemy.potion.name} potion`);
    
      // log the round index to see if all enemies are defeated. 
      this.roundNumber++;
    
      if (this.roundNumber < this.enemies.length) {
        // if more enemy's are left start new battle with next enemy.
        this.currentEnemy = this.enemies[this.roundNumber];
        this.startNewBattle();
      } else {
        // if all dead log you win
        console.log('You win!');
      }
    }
    else {
      // if player was defeated and enemy is still alive
